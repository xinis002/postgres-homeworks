...
SELECT AVG (shipped_date - order_date) as total  FROM orders where ship_country='Germany';
...
SELECT MIN(unit_price), MAX(unit_price) FROM products WHERE discontinued IS NOT NULL; 
...
SELECT MIN(unit_price), MAX(unit_price) FROM products WHERE discontinued IS NOT NULL and units_in_stock > 20; 
